...
* [`src/fetchimagefsm2/`](src/fetchimagefsm2/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/fetchimagefsm2/core.cljs): contains the SPA entry point, `init`
* [`test/fetchimagefsm2/`](test/fetchimagefsm2/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `fetchimagefsm2` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/fetchimagefsm2/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns fetchimagefsm2.example
  (:require [fetchimagefsm2.config :as config])
